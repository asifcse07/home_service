...
class SystemAuthController extends Controller
{
    ...
    public function authRegistration(Request $request)
    {
        ...
        try {
            ...
            return redirect('/#toregister')->with('error-message',"Duplicate email or something is wrong on user registration ! Please try again..");
        }
    }
    ...
    //admin register
    public function adinRegitrationPage(){
    ...
    public function authAdminRegistration(Request $request){
        // print_r($request->all()); die();
        $now = date('Y-m-d H:i:s');
        $v = \Validator::make($request->all(), [
            'company_name' => 'required',
            'company_code' => 'required|min:6|unique:users',
            'password' => 'required|min:6',
            'retype_password' => 'required|same:password',
        ...
        try {
            ...
            if($registration_confirm) {
                return redirect('/admin-login')->with('success-message',"You have successfully registered");
            }
            ...
            $message = "Message : ".$e->getMessage().", File : ".$e->getFile().", Line : ".$e->getLine();
            \App\System::ErrorLogWrite($message);
            return redirect('/admin')->with('error-message',"Duplicate comapny or something is wrong on user registration ! Please try again..");
        }
    }
    ...
    public function authAdminLogin()
    {
        if (\Auth::check()) {
            if(\Auth::user()->role == 1){
                \App\User::LogInStatusUpdate("login");
            return redirect('/dashboard')->with('error-message', 'You have already loggedin !.');
    ...
    //check
    public function authPostAdminLogin(Request $request)
    {
        $validator = \Validator::make($request->all(), [
        ...
        if (\Auth::attempt($credentials)) {
            ...
            \Session::put('company_code', \Auth::user()->company_code);
            \Session::put('last_login', Auth::user()->last_login);

            \App\User::LogInStatusUpdate("admin-login");
            ...
            return redirect('/admin-login')->with('error-message',"Incorrect combinations.Please try again.");
        }
    }
